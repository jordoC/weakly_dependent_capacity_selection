...
h_arr = cell(n,1);
for arr_idx = 1:n
    h_arr{arr_idx,1} = (1/sqrt(4*N))*(randn(rv_len,2*N,1));
    h_arr{arr_idx,1} = reshape(h_arr{arr_idx,1},rv_len*2,N);
end
    ...
    for h_norm_idx = 1:l
        for h_orth_idx = 1:l
            ...
            h_norm_orth_mat(:,h_norm_idx,h_orth_idx) = ...
                sum(((sus_group{h_norm_idx}).*(sus_group{h_orth_idx}))')';
        end
    end
...
for sus_group_idx = 1:num_groups
    ...
    for h_norm_idx = 1:l
        ...
        cap(:,h_norm_idx) = log2(1+sinr(:,h_norm_idx))/2;
    end
